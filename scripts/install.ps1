...
if ($SkipCredentials) {
    ...
    try {
        $tokenResp = Invoke-RestMethod -Uri "https://open.larksuite.com/open-apis/auth/v3/tenant_access_token/internal/" `
    ...
    if ($Phone -or $Email) {
        ...
        try {
            $lookupResp = Invoke-RestMethod `
                -Uri "https://open.larksuite.com/open-apis/contact/v3/users/batch_get_id?user_id_type=open_id" `

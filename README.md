...
```
/plugin install claude-lark from pengxjwawa123/claude-notify-lark
/setup-lark
```

`/setup-lark` 会引导你输入 App ID、App Secret 和手机号，自动完成 Open ID 查询、配置写入和测试通知。

> 如果团队管理员已经给你准备好了配置文件，直接放到 `~/.config/claude-lark/config.json` 即可，跳过 `/setup-lark`。
...
```bash
git clone https://github.com/pengxjwawa123/claude-notify-lark.git
cd claude-lark
